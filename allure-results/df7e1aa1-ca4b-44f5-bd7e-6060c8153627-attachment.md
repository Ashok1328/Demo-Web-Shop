...
Call log:
  - waiting for locator('input[onclick=\'PaymentMethod.save()\'], button[onclick=\'PaymentMethod.save()\'], .payment-method-next-step-button') to be visible
    7 × locator resolved to hidden <input type="button" value="Continue" onclick="PaymentMethod.save()" class="button-1 payment-method-next-step-button"/>
...
```ts
  6   |     readonly onePageCheckoutLink: Locator;
  7   |     readonly billingAddressSelect: Locator;
  8   |     readonly billingAddressFirstName: Locator;
